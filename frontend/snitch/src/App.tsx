...
const Spinner = ({ size = 28 }: { size?: number }) => (
  <svg className="animate-spin text-primary" width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
...
function getInitialStep(user: UserProfile): SetupStep {
  ...
  const cardDone = !!localStorage.getItem('snitch_card_done') || !!user.payment_method_last4
...
function SetupScreen({
  ...
  useEffect(() => {
    if (step !== 'card') return
    apiPost<{ client_secret: string; customer_id: string }>('/payments/setup-intent', {}, token)
  ...
  async function goToConnect() {
    ...
    try {
      ...
      const { url } = await apiPost<{ url: string }>('/connect/onboarding-link', {}, token)
  ...
  const wrap = (children: React.ReactNode) => (
    <div className={OUTER}>
      <div className={INNER}>
        <Shell user={user} onSignOut={onSignOut} screen="dashboard" onNavigate={() => {}} showNav={false}>
          {children}
        </Shell>
      </div>
    </div>
  ...
  if (step === 'refreshing') {
    return wrap(
      <div className="flex flex-col items-center justify-center flex-1 gap-4 h-full">
        <Spinner />
        <p className="font-body text-sm text-on-surface-variant italic">Refreshing setup link…</p>
      </div>
    )
  }
      ...
      <div className="flex flex-col items-center justify-center flex-1 px-6 gap-6 text-center h-full">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="font-body text-sm text-on-surface-variant leading-relaxed max-w-xs">
            Stripe is confirming your payout account. This usually takes a few seconds.
          </p>
        </div>
        ...
        >
          {connectLoading ? 'Redirecting…' : "Didn't finish onboarding? Try again"}
        </button>
      </div>
    )
  }
    ...
    return wrap(
      <div className="flex flex-col flex-1 px-6 py-8 gap-6">
        ...
        {!cardClientSecret ? (
          ...
          <StripeCardForm clientSecret={cardClientSecret} token={token} onSuccess={handleCardSuccess} />
        )}
      </div>
    )
  }
  ...
  return wrap(
    <div className="flex flex-col items-center justify-center flex-1 px-6 gap-6 text-center h-full">
      <div className="flex flex-col items-center gap-2">
        ...
        <p className="font-body text-sm text-on-surface-variant leading-relaxed max-w-xs">
          Complete your Stripe payout account so your friends can receive their winnings
      ...
      <button
        onClick={() => {
          apiGet<UserProfile>('/users/me', token)
            .then(updated => { if (updated.stripe_account_enabled) onSetupComplete(updated) })
            .catch(() => {})
        }}
...
export default function App() {
  ...
  return (
    <div className={OUTER}>
      <div className={INNER}>
        <Shell user={activeUser} onSignOut={signOut} screen={screen} onNavigate={navigate}>
          {screen === 'dashboard' && (
            <FocusDashboard navigate={navigate} token={token} user={activeUser} />
          )}
          {screen === 'stats' && (
            <FocusStats navigate={navigate} token={token} user={activeUser} />
          )}
          {screen === 'monitor' && (
            <DistractionMonitor navigate={navigate} />
          )}
          {screen === 'settings' && (

...
class AuthClient:
    ...
    async def get_discord_account_status(self, discord_uid: int) -> tuple[bool, str, Optional[dict[str, Any]]]:
    ...
    async def authenticate_discord_user(
        ...
        if require_payment_method and not bool(discord_account_status.get('payment_method_ready')):
            return (
                False,
                (f'No saved payment method found.\nConnect one here: {self.frontend_payment_setup_url}'),
                None,
                discord_account_status,
            )
